...
#### revised on: 9 Oct 2024

supermat <- function(infiles,
                     outfile = "supermat.out.fas",
    ...
    for (i in 1:length(infiles)) {
        ...
        if (any(grepl(">", temp))) {
            ### find if a ">" symbol exists, it is fasta file.
    ...
    for (i in 2:ncol(supermat.dat)) {
        ...
        ### times of repeat was determined by the the length of the longest sequence of the same gene.
        supermat.sequences <- paste(supermat.sequences,
                                    ifelse(
                                        is.na(tem.sequences),
                                        paste(rep("-", nsites[i]), collapse = ""),
                                        as.character(tem.sequences)
                                    ),
                                    sep = "")
    }
    ...
    outfile_extension = substr(outfile, nchar(outfile) - 4, nchar(outfile))
    if (grepl("phy", outfile_extension)) {
        dat2phylip(res.super.dat, outfile)
    } else {
        if (grepl("fas", outfile_extension)|grepl("asta", outfile_extension)) {
            writeLines(paste(
                ">",
                as.character(res.super.dat[, 1]),
                "\n",
                as.character(res.super.dat[, 2]),
                sep = ""
            ),
            outfile)
        }
